...
levels_extreme = np.append(np.arange(-3., -0.3, 0.3),
                           -np.arange(-3, -0.3, 0.3)[::-1]) * 3
...
#%% 5. Draw zoom panels -------------------------------------------------------
# Force draw so ax_globe.transData is properly initialised for coordinate transforms
fig.canvas.draw()
...
for name in region_order:
    ...
    for col_j in range(n_sectors):
        ...
        precip_clim = ds['precip_ext'].isel(reg=oi)
        precip_event = ds['precip_llb_ext'].isel(reg=oi)
        pval = ds['pval_llb_ext'].isel(reg=oi)
        anomaly = precip_event - precip_clim

        cf = ax.contourf(ds['lon'], ds['lat'], anomaly,
                         levels=levels_extreme, cmap=cmap, extend='both',
